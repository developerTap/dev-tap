![](https://blog.kakaocdn.net/dn/cc2a91/btsMAsB3Tpe/ylAWSkXCm16dTidxU3RuU0/img.png)
...
모듈이 `greeting-module.js`, `greeting-module.ts`, `index.js` 등 여러 형태로 있을 수 있는데, 컴파일러는 찾은 파일이 **어떤 형식의 모듈인지** 다시 체크해야 한다. [🔍 **2) 모듈의 종류 확인**](https://ironist-tapkim.tistory.com/manage/newpost/24?type=post&returnURL=ENTRY#%EB%AA%A8%EB%93%88%EC%9D%98-%EC%A2%85%EB%A5%98-%ED%99%95%EC%9D%B8)을 다시 수행.
...
4. **올바른 모듈 형식으로 변환:** `ts` 코드를 `js`로 변환할 때, **출력 코드가 유효한 모듈 형식**이어야 함. (`import` → `require()`로 바꿔야 하는지 등)
5. **import가 정상적으로 되는지 확인:** `import hello from "greeting-module";`가 **정상적으로 작동하는지** 체크. (만약 `hello`가 존재하지 않으면 **컴파일 오류 발생!** 🚨)
6. **import한 값의 타입을 할당**
    - `hello`가 함수인지? 객체인지?
