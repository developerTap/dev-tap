...
브라우저에 일부 HTML만 먼저 로드되고 CSS나 JavaScript는 지연되는 경우, 초기 렌더링이 빠르게 이루어지지만 잠시 깨진 화면이 보이고, 이후 리소스가 모두 적용되며 화면이 크게 변화하게 된다. 이러한 방식은 초기부터 빈 화면을 보여주며 필요한 리소스를 충분히 확보한 뒤 렌더링하는 전략보다 사용자 경험(UX)에 더 부정적일 수 있다. 반면, 모든 리소스를 로드할 때까지 렌더링을 지연하는 방식도 마찬가지로 UX에 좋지 않다.
